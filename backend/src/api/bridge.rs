use crate::services::allbridge::{
    supported_chains, AllbridgeClient, AllbridgeQuoteRequest, BridgeStatusKind,
...
/// Request body for initiating an outbound bridge withdrawal (Stellar ΓåÆ external chain).
#[derive(Deserialize)]
pub struct WithdrawBridgeRequest {
    /// Stellar transaction hash of the originating lock transaction.
    pub source_tx_hash: String,
    /// Allbridge chain symbol of the destination chain (e.g. "ETH", "BSC", "SOL").
    pub destination_chain: String,
    /// Recipient wallet address on the destination chain.
    pub destination_address: String,
    /// Token symbol being sent (e.g. "USDC").
    pub token: String,
    /// Amount as a string forwarded verbatim to Allbridge.
    pub amount: String,
}

/// Request body for a withdrawal quote (Stellar ΓåÆ external chain).
#[derive(Deserialize)]
pub struct WithdrawQuoteRequest {
    pub destination_chain: String,
    pub destination_token: String,
    pub amount: String,
    pub destination_address: String,
...
/// GET /api/bridge/chains ΓÇö return all supported bridge chains with their tokens.
pub async fn get_supported_chains() -> impl IntoResponse {
    Json(supported_chains())
}

/// POST /api/bridge/withdraw/quote ΓÇö get a quote for Stellar ΓåÆ external chain.
pub async fn get_withdraw_quote(
    State(state): State<BridgeState>,
    Json(payload): Json<WithdrawQuoteRequest>,
) -> impl IntoResponse {
    if payload.destination_chain.trim().is_empty()
        || payload.destination_token.trim().is_empty()
        || payload.amount.trim().is_empty()
        || payload.destination_address.trim().is_empty()
    {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({
                "error": "destination_chain, destination_token, amount and destination_address are required"
            })),
        )
            .into_response();
    }
    ...
    let quote_request = AllbridgeQuoteRequest {
        ...
        source_token: payload.destination_token.clone(), // same token, Stellar side
        amount: payload.amount,
        destination_chain: payload.destination_chain,
        destination_token: payload.destination_token,
        destination_address: payload.destination_address,
...
/// POST /api/bridge/withdraw ΓÇö record and initiate an outbound Stellar ΓåÆ external chain transfer.
pub async fn submit_withdraw_tx(
    State(state): State<BridgeState>,
    Json(payload): Json<WithdrawBridgeRequest>,
) -> impl IntoResponse {
    if payload.source_tx_hash.trim().is_empty() {
        return (
            StatusCode::BAD_REQUEST,
    ...
    if payload.destination_chain.trim().is_empty() || payload.destination_address.trim().is_empty()
    {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({
                "error": "destination_chain and destination_address are required"
            })),
        )
            .into_response();
    }

    // Build the Allbridge withdrawal request: source is always Stellar.
    let withdraw_request = AllbridgeQuoteRequest {
        source_chain: "STLR".to_string(),
        source_token: payload.token.clone(),
        amount: payload.amount.clone(),
        destination_chain: payload.destination_chain.clone(),
        destination_token: payload.token.clone(),
        destination_address: payload.destination_address.clone(),
    };

    // Call Allbridge to register the outbound transfer.
    let bridge_tx_data = match state.allbridge.initiate_withdrawal(&withdraw_request).await {
        Ok(resp) => resp.bridge_tx_data,
        Err(err) => {
            tracing::error!("Allbridge withdrawal initiation failed: {:?}", err);
            return (
                StatusCode::BAD_GATEWAY,
                Json(serde_json::json!({ "error": err.message })),
            )
    ...
    // Persist the withdrawal in PENDING state (idempotent on hash collision).
    let result = sqlx::query(
        r#"
        INSERT INTO bridge_transactions
            (source_tx_hash, source_chain, destination_chain, destination_address, amount, status, direction)
        VALUES ($1, 'STLR', $2, $3, $4, 'PENDING', 'OUTBOUND')
        ON CONFLICT (source_tx_hash) DO UPDATE SET updated_at = NOW()
        RETURNING id, status
        "#,
    )
    ...
    match result {
        Ok(row) => {
            ...
            Json(serde_json::json!({
                ...
                "destination_address": payload.destination_address,
                "status": status,
                "bridge_tx_data": bridge_tx_data,
            }))
            .into_response()
        }
        Err(e) => {
            tracing::error!("Failed to record withdrawal transaction: {:?}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({ "error": "Failed to record withdrawal transaction" })),
            )
                .into_response()
        }
    }
