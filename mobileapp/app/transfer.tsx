...
function TransferScreen() {
  ...
  const [transferType, setTransferType] = useState<"BEAMPAY" | "external" | null>(
    "BEAMPAY"
  );
  ...
  const [searchResults, setSearchResults] = useState<BeamPayUser[]>([]);
  ...
  const searchUsers = useCallback(async (query: string) => {
    ...
    try {
      ...
      const data: BeamPayUser[] = await res.json();
  ...
  // Debounce search while user types (BEAMPAY mode only)
  useEffect(() => {
    if (transferType !== "BEAMPAY") return;
  ...
  const handleSelectUser = useCallback((user: BeamPayUser) => {
  ...
  const renderStep0 = () => (
    <View style={styles.stepContainer}>
      ...
      <View style={styles.cardsContainer}>
        <AccountTypeCard
          title="BeamPay User"
          description="Send instantly to any BeamPay User via their BEAMPAY ID"
          Icon={BeamPayIcon}
          selected={transferType === "BEAMPAY"}
          onPress={() => setTransferType("BEAMPAY")}
        />
        <AccountTypeCard
  ...
  const renderStep1 = () => (
    <View style={styles.stepContainer}>
      <View style={styles.inputsSection}>
        {/* Recipient input + live-search dropdown (BEAMPAY mode only) */}
        <View style={styles.recipientSearchWrapper}>
          <Input
            placeholder={
              transferType === "BEAMPAY"
                ? "Recipient BEAMPAY ID (e.g. tolu.BEAMPAY)"
                : "Wallet Address"
            }
            value={recipient}
          ...
          />
          {/* Searching indicator */}
          {transferType === "BEAMPAY" && searching && (
            <ActivityIndicator
              size="small"
              color="#1A4B4A"
      ...
      <View style={styles.visibilitySection}>
        ...
        <Text style={styles.visibilityDesc}>
          {visibility === "PUBLIC" && "Visible to anyone on the BEAMPAY network."}
      ...
      <View style={styles.summaryCardLarge}>
        ...
        <View style={styles.infoRow}>
          <View style={styles.recipientBadge}>
            <BeamPayIcon width={16} height={16} />
          </View>
