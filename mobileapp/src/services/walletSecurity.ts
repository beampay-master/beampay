 ...
 * walletSecurity.ts
 *
 * Secure key management service for the BeamPay wallet.
 *
 * Security guarantees:
...
import { hmacSha512 } from "../utils/hmac";
import { Platform } from "react-native";
import AsyncStorage from "@react-native-async-storage/async-storage";

// ── Secure storage abstraction (SecureStore on native, AsyncStorage on web) ──
// Note: web fallback uses AsyncStorage which is NOT encrypted.
// For production web support, integrate a proper web keystore solution.
...
export async function deriveKeypairFromMnemonic(
  ...
  // Ensure Buffer is available globally (needed on web/Hermes)
  ...
  const derivedKey = await deriveStellarKey(seed, accountIndex);
 ...
 */
async function deriveStellarKey(seed: Buffer, accountIndex: number): Promise<Buffer> {
  const masterResult = await hmacSha512(
    Buffer.from("ed25519 seed", "utf8"),
    seed
  );
  ...
  for (const index of pathComponents) {
    ...
    const result = await hmacSha512(chainCode, data);
...
export async function storeMnemonic(mnemonic: string): Promise<void> {
  await secureSet(MNEMONIC_KEY, mnemonic);
}
...
export async function storeKeypair(
  ...
  await secureSet(secretKeyStorageKey(accountIndex), keypair.secret());
  await secureSet(publicKeyStorageKey(accountIndex), keypair.publicKey());
}
...
export async function retrieveKeypair(
  ...
  const secret = await secureGet(secretKeyStorageKey(accountIndex));
...
export async function retrievePublicKey(
  ...
  return secureGet(publicKeyStorageKey(accountIndex));
}
...
export async function getAccountCount(): Promise<number> {
  const raw = await secureGet(ACCOUNT_COUNT_KEY);
...
async function incrementAccountCount(): Promise<number> {
  ...
  await secureSet(ACCOUNT_COUNT_KEY, String(next));
  return next;
}
...
export async function createNewWallet(
  ...
  await secureSet(ACCOUNT_COUNT_KEY, "1");
...
export async function deleteWallet(): Promise<void> {
  const count = await getAccountCount();
  for (let i = 0; i < count; i++) {
    await secureDelete(secretKeyStorageKey(i));
    await secureDelete(publicKeyStorageKey(i));
  }
  await secureDelete(MNEMONIC_KEY);
  await secureDelete(ACCOUNT_COUNT_KEY);
}
