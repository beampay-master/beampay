...
const NOTIFICATION_PREFERENCE_KEY = "beampay_notifications_enabled";
const PUSH_TOKEN_KEY = "beampay_push_token";
  ...
  try {
    await fetch(`${API_BASE}/notifications/register`, {
      ...
      body: JSON.stringify({
        ...
        appId: Constants.manifest?.slug || Constants.expoConfig?.slug || "beampay",
      }),
    });
  } catch {
...
> {
  ...
  if (status !== Notifications.PermissionStatus?.GRANTED && status !== "granted") {
    return null;
  }
